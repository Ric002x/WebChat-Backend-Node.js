...
export const registerSchema = z.object({
    ...
    email: z.email({ error: "Campo obrigatório" }),
    password: z.string({ error: "Campo obrigatório" }).regex(/^(?=.*[A-Z])(?=.*\d).{8,}$/, { error: "A senha deve ter o mínimo de 8 caracteres, e conter ao menos um número e um caractere especial" }),
